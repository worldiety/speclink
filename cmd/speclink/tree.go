...
type treeReport struct {
	...
	Findings     []diag.JSONFinding  `json:"findings,omitempty"`
}
...
type treeRequirement struct {
	...
	//
	// Empty when the run did not load the implementation, which is the normal
	// case for `requirements`: that command deliberately reads no annotation so
	// the tree can be worked on while the code around it is in pieces. It is
	// also the case that matters, because the audience for this export is a
	// person judging whether a requirement says what they meant, and the name
	// of the Go type implementing it tells them nothing.
...
func writeTree(w io.Writer, root string, tree *reqtree.Tree, set *source.Set, docs []string, cov check.SourceCoverage, sat map[string][]ir.Target, base *baseline.File, findings *diag.Set) error {
	...
	// The same shape the findings report uses, because two shapes for one
	// concept would be two contracts to keep in step. Paths are rewritten for
	// the same reason as everywhere else in this export.
	report.Findings = diag.JSONFindings(findings.Findings())
	for i := range report.Findings {
		report.Findings[i].File = relTo(root, report.Findings[i].File)
	}
